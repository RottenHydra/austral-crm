...
export default function ContactsTable({ rows }) {
  ...
  const filtered = data.filter(r => {
    ...
    if (q) {
      const s = (r.company + ' ' + (r.city || '') + ' ' + (r.segment || '')).toLowerCase();
  ...
  return (
    <>
      <div className="toolbar">
        <input placeholder="Search company / city / segment" value={q} onChange={e => setQ(e.target.value)} style={{ minWidth: 260 }} />
        ...
        <table>
          <thead>
            <tr>
              ...
              <th>Status</th><th>Response</th><th>Next step</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map(r => (
              <tr key={r.company}>
                ...
                <td className="chan">
                  ...
                  {r.touches ? <div>{r.touches} touch{r.touches > 1 ? 'es' : ''}</div> : null}
                </td>
